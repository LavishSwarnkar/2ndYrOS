//SJF(NP) Priority Algo
#include<stdio.h>
#include<conio.h>
struct process
{ 	int P, AT, BT, CT, TA, WT, done;
};
int getShortestJob(struct process pro[20], int time, int size)
{   int i, j=-1, tmp=-1;
	for(i=1 ; i<=size ; i++)
	{	if(pro[i].done==0 && pro[i].AT<=time && pro[i].P>tmp)
		{   tmp = pro[i].P;
			j=i;
		}
	}
...
{	int size, i, time, j;
	...
	printf("\nProcess\tP\tAT\tBT\tCT\tTA\tWT\n");
	for(i=1 ; i<=size ; i++)
		printf("P%d\t%d\t%d\t%d\t%d\t%d\t%d\n", i, pro[i].P, pro[i].AT, pro[i].BT, pro[i].CT, pro[i].TA, pro[i].WT);
